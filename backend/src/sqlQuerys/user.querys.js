...
function SELECT_USERS_AND_LIKES() {
  let sql = '';
  sql += 'SELECT username, role, SUM(if(likedPostId=0,0,1)) AS likes ';

  sql += 'FROM ( ';
  sql += '       SELECT username, role, COALESCE(postId, 0) as likedPostId ';
  sql += '       FROM user ';
  sql += '       left join postLikedBy ';
  sql += '       on user.id=postLikedBy.userId ';
  sql += ') as myTable3 ';
  sql += 'GROUP BY username, role, likedPostId ';
